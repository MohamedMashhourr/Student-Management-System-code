...
//*****************************thired choice to delete student using id*********************************//
        else if(x==3)
        {
            ...
            scanf("%d",&id);
            test.id=id;
            state=student_delete(&head,test);
            ...
            while(1)
            {
              ...
              //  printf("      2- Find Using Name\n");
                ...
                if(z==1) //searching using id
                {
                    ...
                    scanf("%d",&id);
                    test.id=id;
                    state= student_SearchId(head,test); //compare the input id with STUDENTS id
...

                    {
                        while(1)
                        {
                            ...
                            scanf("%d",&y);
                            if(y==1) // Print the details of the student for prev ID
                            {
                                student_PrintId(head,test);
                                break;
                            }
                            if(y==2) //edit the id of the student
                            {
                                printf(" Enter The New ID Number:  ");
                                scanf("%d",&test.id);
                                student_EditId(head,s,test);
                                break;
